...
;               al = A0h for master, B0h for slave
    ...
    mov byte [rbx + fdiskTblEntry.signature], 0    ;Clear signature byte in table
    mov word [rbx + fdiskTblEntry.ioBase], dx      ;Add iobase and masterslave status
    mov byte [rbx + fdiskTblEntry.msBit], al       
;CHS, none of CHS is allowed to be 0 but may be because obsolete on new drives
    mov ax, word [rdi + idCurrCyl]
    test ax, ax
    jz .id0
    mov word [rbx + fdiskTblEntry.wCylinder], ax
    mov ax, word [rdi + idCurrHed]
    test ax, ax
    jz .id0
    mov word [rbx + fdiskTblEntry.wHeads], ax
    mov ax, word [rdi + idCurrSecTk]
    test ax, ax
    jz .id0
    mov word [rbx + fdiskTblEntry.wSecTrc], ax
.id0:
;LBA28
    ...
    or byte [rbx + fdiskTblEntry.signature], fdeLBA28 ;Set LBA28 present bit
    mov dword [rbx + fdiskTblEntry.lbaMax], eax
.id1:
;LBA48
;Check LBA48 bit first
    ...
    mov qword [rbx + fdiskTblEntry.lbaMax48], rax
    or byte [rbx + fdiskTblEntry.signature], fdeLBA48
.id2:
;Now check if either LBA28 or LBA48 are set or CHS is non-zero
    test byte [rbx + fdiskTblEntry.signature], fdeLBA28 | fdeLBA48
    ...
    movzx eax, word [rbx + fdiskTblEntry.wCylinder]
    test eax, eax
    ...
    cmp ax, 07F7Fh
    je .id3
    movzx eax, word [rbx + fdiskTblEntry.wHeads]
    test eax, eax
    jz .id3 ;If zero, dont confirm device
    cmp ax, 0FFFFh
    je .id3
    cmp ax, 07F7Fh
    je .id3
    movzx eax, word [rbx + fdiskTblEntry.wSecTrc]
    test eax, eax
    jz .id3
    cmp ax, 0FFFFh
    je .id3
...
;Clean any data that mightve been copied (from Floating Bus reads perhaps)
    xor eax, eax
    mov byte [rbx + fdiskTblEntry.signature], al
    mov dword [rbx + fdiskTblEntry.lbaMax], eax
    mov qword [rbx + fdiskTblEntry.lbaMax48], rax
    mov word [rbx + fdiskTblEntry.wCylinder], ax
    mov word [rbx + fdiskTblEntry.wHeads], ax
    mov word [rbx + fdiskTblEntry.wSecTrc], ax
    mov word [rbx + fdiskTblEntry.ioBase], ax
    mov byte [rbx + fdiskTblEntry.msBit], al
    jmp short .idExit
.idDeviceOK:
    or byte [rbx + fdiskTblEntry.signature], fdePresent
...
;Output: CF=NC, all ok, CF=CY, device not added.
    push rsi
    cmp byte [ideNumCtrlr], 2
    je .actfail ;If it is 2, fail
    inc byte [ideNumCtrlr]
    mov rsi, ideCtrlrTbl
    cmp byte [rsi], 0   ;Is the first entry empty?
    jz .act0    ;If yes, write entry
    add rsi, ideCtrlrTblEntry_size  ;Else, goto second entry space
.act0:
